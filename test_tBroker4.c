...
void *All_topic_sub0_func(void *par) 
{
    ...
    struct tBroker_subscriber_context *fd_00_ctx = NULL, *fd_01_ctx = NULL, *fd_02_ctx = NULL, *fd_03_ctx = NULL;
    ...
    if (epoll_fd > 0) {
        ...
        printf("4. fd_00_count = %d, fd_01_count = %d, fd_02_count = %d, fd_03_count = %d \r\n", fd_00_count, fd_01_count, fd_02_count, fd_03_count);
        ...
        if (fd_00_ctx) tBroker_subscriber_context_free(fd_00_ctx);
        if (fd_01_ctx) tBroker_subscriber_context_free(fd_01_ctx);
        if (fd_02_ctx) tBroker_subscriber_context_free(fd_02_ctx);
        if (fd_02_ctx) tBroker_subscriber_context_free(fd_03_ctx);
