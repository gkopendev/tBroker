 ...
 * @arg ctx: returned by previous  topic_subscribe call
 * @arg buffer: data to be copied. It is pointer to data of memory 
 * associated with the topic
 * @return: Number of bytes read and -1 if no new data available
 */
int tBroker_topic_read(int topic, struct tBroker_subscriber_context *ctx, 
							void *buffer);

/*
 * @desc Same as above but only read topic data at @arg offset upto @arg 'size' 
 * bytes. One difference is unlike tBroker_topic_read, it will return last valid
 * data which could be unread or was read previously. Useful for multiple calls 
 * to same data structure.
