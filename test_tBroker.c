...
void *All_topic_pub_func1(void *par)
{
    ...
    while (times < LOOP) {
     ...
     //   tBroker_topic_publish(TOPIC_4, dummy_pub_data);
        times++;
        nanosleep(&ts, NULL);
    }
    sleep(5);
...
void *All_topic_pub_func2(void *par)
{
    ...
    while (times < LOOP) {
     ...
     //   tBroker_topic_publish(TOPIC_5, dummy_pub_data);
        times++;
        nanosleep(&ts, NULL);
    }
     sleep(5);
...
void *All_topic_pub_func3(void *par)
{
    ...
    while (times < LOOP) {
     ...
     //   tBroker_topic_publish(TOPIC_6, dummy_pub_data);
        times++;
        nanosleep(&ts, NULL);
    }
     sleep(5);
...
void main(void)
{
  ...
  //  schedParam.sched_priority = sched_get_priority_max(SCHED_FIFO) - 1;
    ...
    if (tBroker_topic_create(TOPIC_0, "First topic", BUF_SIZE, Q_SIZE) >= 0) {  /* Add TOPIC_0 to the broker */
        ;
    }
    
    /* Add TOPIC_1 and add 4 subscribers */
    if (tBroker_topic_create(TOPIC_1, "Second topic", BUF_SIZE, Q_SIZE) >= 0) {
        ;
    }
    
    /* Add TOPIC_2 and add 4 subscribers */ 
    if (tBroker_topic_create(TOPIC_2, "Third topic", BUF_SIZE, Q_SIZE) >= 0) {
        ;
    }
    
    /* Add TOPIC_3 and add 4 subscribers */
